...
inline void
blur_unit(uchar *input, uchar *output, const int blur_size, const int width, const int height, int row, int col) {
    int pix_val = 0;
    int pixels = 0;
...
void ch3__blur_device(uchar *h_input, uchar *h_output, const int blur_size, const int width, const int height,
...
void ch3__blur(env_e env, kernel_config_t config) {
    ...
    Mat src = cv::imread(CH3__INPUT_FILE_BLUR, cv::IMREAD_GRAYSCALE);
